...
func (s *Server) handleShot(w http.ResponseWriter, r *http.Request) {
	...
	default:
		// A caller that pins v= to a cache key gets an immutable answer; one
		// that does not gets a short revalidating one. Both are correct, and
		// the difference is the caller's to choose.
		...
		if entry != nil && entry.Freshness != "" {
			h.Set("ETag", `"`+entry.Freshness[:16]+`-`+rung.Name+`"`)
